...
import { renderWithReduxAndRouter, getTestProduct, userEvent } from "../../../utils/test-utils";
...
describe("<ItemCard />", () => {
	...
	it("renders the card img", () => {
		...
		const { getByRole } = renderWithReduxAndRouter(
			<ItemCard id={product.id} img={product.img} title={product.title} shortDescription={product.shortDescription} upVotes={product.votes.upVotes} handleUpVote={handleUpVote} downVotes={product.votes.downVotes} handleDownVote={handleDownVote} isFavorite={product.isFavorite} handleSetFavorite={handleSetFavorite} handleAddCartItem={handleAddCartItem} />
		);
	...
	it("calls the card add to cart button callback with the id on click", () => {
		...
		expect(handleAddCartItem).toHaveBeenCalledWith(product.id);
	});
